...
votations = {}
CSV.foreach(Rails.root.join('db/vote_info_cn.csv'), headers: true) do |row|
...
# Read the people CSV file and create the corresponding database records
CSV.foreach(Rails.root.join('db', 'ch_nr_par.csv'), headers: true) do |row|
  person = Person.new(
